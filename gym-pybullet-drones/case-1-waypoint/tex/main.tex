...
\begin{abstract}
This document focuses on the simulation results, analysis and corresponding optimizations of a pid based simple ``Hybrid UAV Waypoint Tracking'' (square climbing) task simulated by \cite{panerati2021learning}.
\end{abstract}
...
\begin{itemize}
  ...
  \item Physical state is governed jointly by the underlying \textit{rigid body dynamics} and the \textit{closed-loop PID controller}:
\end{itemize}

\begin{equation}
\dot{x} = f(x) = \begin{bmatrix} 
v \\
\mathcal{F}_{dyn}(p, v, u_{pid}(p, v, W_q)) \\ 
0 
\end{bmatrix}, \quad x \in C
\end{equation}
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=0.95\textwidth]{fig/gui_opt.png}
